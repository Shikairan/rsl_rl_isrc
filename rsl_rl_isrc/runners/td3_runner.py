...
#
"""TD3 训练运行器：off-policy 逐步采集、ReplayBuffer 更新与分布式广播。"""
...
class TD3Runner:
    """封装 TD3 训练：构建 ``TD3Networks`` 与 ``TD3`` 算法、管理 ``ReplayBuffer``、日志与检查点。"""
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        td3_networks.set_action_bounds(low, high)

        self.alg = TD3(
            td3_networks=td3_networks,
            device=self.device,
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        if dist.is_initialized():
            for param in self.alg.td3_networks.parameters():
    ...
    def save(self, path, infos=None):
        torch.save(
            {
                "td3_networks": self.alg.td3_networks.state_dict(),
    ...
    def load(self, path, load_optimizer=True):
        ...
        self.alg.td3_networks.load_state_dict(loaded["td3_networks"])
        if load_optimizer:
    ...
    def get_inference_policy(self, device=None):
        ...
        if device is not None:
            self.alg.td3_networks.to(device)
        return self.alg.td3_networks.act_inference
