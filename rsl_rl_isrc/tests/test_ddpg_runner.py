#!/usr/bin/env python3
"""DDPG Runner 测试：Gymnasium Pendulum-v1 短程训练冒烟验证。"""
...
from rsl_rl_isrc.runners import DDPGRunner


class GymnasiumVecEnv(VecEnv):
    """将 Gymnasium 向量环境适配为 ``VecEnv`` 接口（用于 DDPGRunner 测试）。"""
...
def build_ddpg_pendulum_train_cfg() -> dict:
    """Pendulum-v1 连续控制任务的 DDPG 短程训练配置。"""
    return {
        "runner": {
            "experiment_name": "ddpg_pendulum_test",
...
class TestDDPGRunner(unittest.TestCase):
    """DDPG + DDPGRunner + Gymnasium Pendulum 短程测试。"""
    ...
    def test_ddpg_short_training_pendulum(self):
        """短程 DDPG 训练：验证 DDPGRunner 在 Pendulum 上不崩溃并完成更新。"""
        num_envs = 4
        num_iterations = 5
        steps_per_iter = 32

        env = GymnasiumVecEnv("Pendulum-v1", num_envs=num_envs, device=str(self.device))
        train_cfg = build_ddpg_pendulum_train_cfg()
        ...
        print(
            f"DDPG Pendulum 短程训练通过: "
            f"{num_iterations} iters × {steps_per_iter} steps × {num_envs} envs"
        )

    def test_ddpg_algorithm_type(self):
        """验证 DDPGRunner 构建的是 DDPG 算法实例。"""
        ...
        with tempfile.TemporaryDirectory() as log_dir:
            runner = DDPGRunner(env=env, train_cfg=train_cfg, log_dir=log_dir, device="cpu")
            self.assertIsInstance(runner.alg, DDPG)
