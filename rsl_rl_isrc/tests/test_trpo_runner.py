#!/usr/bin/env python3
"""
使用 TRPORunner 的 TRPO 训练测试
基于 rsl_rl 框架，使用 TRPOPolicy 和正确的 Runner 接口
"""
...
class DummyVecEnv(VecEnv):
    """简化的连续控制向量环境实现，用于测试（Pendulum-like）"""
...
def run_trpo_with_runner(num_episodes=50, num_envs=4, device='cpu'):
    """
    使用 TRPORunner 运行 TRPO 训练
    """
    print("开始 TRPO Runner 训练测试...")
    print(f"设备: {device}")
    ...
    train_cfg = {
        "runner": {
            "experiment_name": "trpo_pendulum_runner_test",
            "num_steps_per_env": 512,  # TRPO 典型的 rollout 长度
            "save_interval": 50
        },
        "algorithm": {
            "max_kl": 0.05,          # KL 约束
    ...
    with tempfile.TemporaryDirectory() as log_dir:
        ...
        for _ in range(10):  # 10 个评估步骤
            with torch.no_grad():
                actions = runner.get_inference_policy()(obs, critic_obs=None)
