...
#
"""DQN 训练运行器：off-policy 逐步采集、DiscreteReplayBuffer 更新与分布式广播。"""
...
class DQNRunner:
    """封装 DQN 训练：构建 ``DQNNetworks`` 与 ``DQN`` 算法、管理离散经验回放。"""
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        dqn_networks = DQNNetworks(
            ...
            hidden_dims=self.policy_cfg.get("hidden_dims", [256, 256]),
            activation=self.policy_cfg.get("activation", "relu"),
        )

        self.alg = DQN(
            dqn_networks=dqn_networks,
            device=self.device,
            **self.alg_cfg,
        )
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        if dist.is_initialized():
            for param in self.alg.dqn_networks.parameters():
            ...
            for _ in range(self.num_steps_per_env):
                ...
                if actions is None:
                    actions = torch.randint(
                        0, self.env.num_actions, (self.env.num_envs, 1), device=self.device
                    )
                else:
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar("Loss/q_value", locs.get("q_loss", 0.0), locs["it"])
        self.writer.add_scalar("Exploration/epsilon", self.alg.current_epsilon(), locs["it"])
        ...
        if len(locs.get("rewbuffer", [])) > 0:
            log_string = (
                ...
                f"{'Q loss:':>{pad}} {locs.get('q_loss', 0.0):.4f}\n"
                f"{'Epsilon:':>{pad}} {self.alg.current_epsilon():.3f}\n"
    ...
    def save(self, path, infos=None):
        torch.save(
            {
                "dqn_networks": self.alg.dqn_networks.state_dict(),
    ...
    def load(self, path, load_optimizer=True):
        ...
        self.alg.dqn_networks.load_state_dict(loaded["dqn_networks"])
        if load_optimizer:
            self.alg.optimizer.load_state_dict(loaded["optimizer"])
    ...
    def get_inference_policy(self, device=None):
        ...
        if device is not None:
            self.alg.dqn_networks.to(device)

        def policy(obs):
            if not torch.is_tensor(obs):
                obs = torch.tensor(obs, dtype=torch.float32, device=device or self.device)
            with torch.no_grad():
                return self.alg.dqn_networks.act_inference(obs)
