...
运行::
    ...
    python rsl_rl_isrc/tests/test_ppo_g1_isaac_cpu.py --num-envs 32 --max-iterations 20

短训验证::

    python rsl_rl_isrc/tests/test_ppo_g1_isaac_cpu.py --num-envs 16 --max-iterations 5 --no-zmq-obs

ObsInstrServer（与 GPU 版相同端口）::

    python rsl_rl_isrc/tests/test_ppo_g1_isaac_cpu.py --num-envs 16 --max-iterations 5 --print-obs
"""
...
def _normalize_device(device: str) -> str:
    ...
    if d in ("cpu", "cuda", "cuda:0"):
        return "cpu" if d == "cpu" else device.strip()
    return device.strip()


def run_g1_isaac_training_cpu(
    num_envs: int,
    max_iterations: int,
    log_dir: str | None,
    ...
    """使用 Isaac G1 环境与 G1OnPolicyTestRunner 在 CPU 上进行 PPO 训练。"""
    _check_isaac()
    ...
    print(
        f"开始训练 (CPU 版): num_envs={num_envs}, max_iterations={max_iterations}, "
        f"log_dir={log_dir}, sim_device={device}, runner.device={device}"
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="G1 Isaac Gym PPO 训练（CPU 仿真/策略，G1OnPolicyTestRunner）"
    )
    parser.add_argument(
        "--num-envs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="TensorBoard / checkpoint 目录；默认 logs/g1/<timestamp>_cpu",
    )
    parser.add_argument(
        "--log-root",
        type=str,
