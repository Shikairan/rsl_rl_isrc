...
def run_trpo_training(num_episodes=100, num_envs=4, hidden_dim=64, learning_rate=1e-3, gamma=0.995, device='cpu', episodes_per_update=10):
    ...
    with tqdm(total=num_updates, desc="TRPO训练进度") as pbar:
        for update in range(num_updates):
            # 收集一个完整的rollout（2048步），跨episode边界
            ...
            # 在test_trpo_training.py的update()调用前后添加
            ...
            # 输出详细更新信息
            print(f"TRPO更新 - 策略损失: {policy_loss:.4f}, 价值损失: {value_loss:.1f}, "
...
if __name__ == '__main__':
    ...
    try:
        results = run_trpo_training(
            num_episodes=100000,  # 减少训练轮数用于快速测试
            num_envs=32,  # 减少环境数量以节省内存
            device=device
        )
        ...
        if results['success']:
            print("🎉 TRPO 成功学习了Pendulum控制任务!")
        else:
            print("⚠️  TRPO 达到了基本性能，但可能需要更多训练")
